...
func (Pages *Pages) updatePageMetadata(oldPageMetadata pageMetadata, dataSize int) error {
	...
	err := Pages.WriteBytes(buf, 0, oldPageMetadata.pageID)
	if err != nil {
...
func formatPageMetadata(metadataBytes []byte) pageMetadata {
	return pageMetadata{
		...
		numSlots:       binary.BigEndian.Uint16(metadataBytes[numEntries:freeSpaceStartIndex]),
...
func CreateMetadataBuffer(pageMetadata pageMetadata) []byte {
	...
	binary.BigEndian.PutUint16(buf[numEntries:freeSpaceStartIndex], pageMetadata.numSlots)
